...
```{r}
#### somewhere here we are losing block 695, is present in CPFV_pelagics but not after this step. Needs to be included in analysis 
CPFV_pelagics$block_id<-as.numeric(as.character(CPFV_pelagics$block_id))
...
```{r}
SoCal_year_block_CPUE<-CPFV_pelagics_socal_rm %>%
...
```{r}
SoCal_year_block_boat<-CPFV_pelagics_socal_rm %>%
  group_by(year, block_id)%>%
  summarise(unique_vessels= n_distinct(id_vessel))
SoCal_year_block_boat_redact<- SoCal_year_block_boat %>% subset(unique_vessels<4)
SoCal_year_block_boat_redact<-subset(SoCal_year_block_boat_redact, select= -unique_vessels)
SoCal_year_block_boat_redact$redact<-"Y"
```
...
complete_pelagicssocal_year_block_rm<-tidyr::complete(ungroup(SoCal_year_block_CPUE), block_id, year, fill=list(average_cpue_p=0, total_caught=0))
...
complete_pelagicssocal_year_block_rm$year<-as.character(complete_pelagicssocal_year_block_rm$year)
SoCal_year_block_boat_redact$year<-as.character(SoCal_year_block_boat_redact$year)
complete_pelagicssocal_year_block_rm_redact<-left_join(complete_pelagicssocal_year_block_rm,
                                          SoCal_year_block_boat_redact,  
                                          by= c("block_id","year"), 
                                          relationship= "many-to-many", 
                                          na_matches= "never")
complete_pelagicssocal_year_block_rm_redact$year<-as.character(complete_pelagicssocal_year_block_rm_redact$year) 
complete_pelagicssocal_year_block_rm_redact$redact[is.na(complete_pelagicssocal_year_block_rm_redact$redact)]<-"N"
complete_pelagicssocal_year_block_rm_redact$total_caught[complete_pelagicssocal_year_block_rm_redact$redact=="Y"]<-NA
complete_pelagicssocal_year_block_rm_redact$average_cpue_p[complete_pelagicssocal_year_block_rm_redact$redact=="Y"]<-NA

##joining shapefile and data
SoCal_redaction_blocks_pelagic<-left_join(SoCal_fishing_blocks,complete_pelagicssocal_year_block_rm_redact,  by= "block_id", relationship= "many-to-many", na_matches= "never")
SoCal_redaction_blocks_pelagic$year<-as.character(SoCal_redaction_blocks_pelagic$year)
```
...
###Path
```{r}
#path_plot_heatmap<-"\\\\10.74.13.28/ci-users/Vanessa.Van.Deusen/R_projects/CPFV_pelagics/Data/graphs/SoCal/SoCal_heatmaps_by_year/independent_scale/CPUE_fish_per_person_no_redact"
path_plot_heatmap<-"\\\\10.74.13.28/ci-users/Vanessa.Van.Deusen/R_projects/CPFV_pelagics/Data/graphs/SoCal/SoCal_heatmaps_by_year/independent_scale/CPUE_fish_per_person_redact"
```
...
#with redaction
SoCal_redaction_blocks_pelagic$year<-as.character(as.factor(SoCal_redaction_blocks_pelagic$year))
SoCal_fishing_blocks_pelagic_pre<- subset(SoCal_redaction_blocks_pelagic, year<=2002)

##Graphing and saving
for(i in unique(SoCal_fishing_blocks_pelagic_pre$year)){
  x<-ggplot()+
    geom_sf(data=SoCal_fishing_blocks, fill="white")+
    geom_sf(data= subset(SoCal_fishing_blocks_pelagic_pre, year==i & year<=2002), aes(fill=average_cpue_p))+
    scale_fill_gradientn(colours=rev(magma(6)),
                         name="Average catch per unit effort \n(fish caught/fisher)",
                         na.value = "grey", limits= c(0, 14))+
    theme_classic()+
    ...
    xlab("Longitude")+
    ylab("Latitude")+
    ggtitle(label =  "Southern California CPFV Catch Per Unit Effort (fish caught/fishers)", 
        subtitle= paste("Year", i, sep = "-"))

  ggsave(x, path= path_plot_heatmap, file=paste0("SoCal_heatmap_individual_average_cpue_fish_per_person", i, ".png") )
}
```
...
#With redaction
SoCal_fishing_blocks_pelagic_2003<- subset(SoCal_redaction_blocks_pelagic, year>=2003 & year<=2004)
...
#With redaction
SoCal_fishing_blocks_pelagic_2005<- subset(SoCal_redaction_blocks_pelagic, year>=2005 & year<=2006)
...
#With redaction
SoCal_fishing_blocks_pelagic_2007<- subset(SoCal_redaction_blocks_pelagic, year>=2007 & year<=2011)
...
for(i in unique(SoCal_fishing_blocks_pelagic_2007$year)){
  x<- ggplot()+
      geom_sf(data=SoCal_fishing_blocks, fill="white")+
      geom_sf(data= subset(SoCal_fishing_blocks_pelagic_2007, year==i & year>2006), aes(fill=average_cpue_p))+
      geom_sf(data=MPA_socal_2003, color="black", fill= "darkblue", alpha=0.25)+
      geom_sf(data=MPA_socal_2005, color="black", fill= "darkblue", alpha=0.25)+
...
#With redaction
SoCal_fishing_blocks_pelagic_2012<- subset(SoCal_redaction_blocks_pelagic, year>=2012)

##Graphing and printing
for(i in unique(SoCal_fishing_blocks_pelagic_2012$year)){
  x<- ggplot()+
      geom_sf(data=SoCal_fishing_blocks, fill="white")+
      geom_sf(data= subset(SoCal_fishing_blocks_pelagic_2012, year==i & year>=2012), aes(fill=average_cpue_p))+
      geom_sf(data=MPA_socal_2003, color="black", fill= "darkblue", alpha=0.25)+
      geom_sf(data=MPA_socal_2005, color="black", fill= "darkblue", alpha=0.25)+
      geom_sf(data=MPA_socal_2007, color="black", fill= "darkblue", alpha=0.25)+
