...
```{r}
##Facet wrapped 
pelagics_socal_species_year_type_rm$year<-as.character(pelagics_socal_species_year_type_rm$year)
...
ggplot()+ 
  geom_line(data=pelagics_socal_species_year_type_rm, aes(x=year, y=CPUE),size=1, color= "black")+
...
ggsave("SoCal_line_pelagic_strong_pdo.jpeg", width= 30, height= 20, units= "cm", path = path_plot)
```
...
ggplot()+ 
  ...
  annotate("rect", xmin = 2018, xmax = 2020, ymin = -Inf, ymax = Inf, fill= "blue",
        alpha = .1)
ggsave("SoCal_line_pelagic_all_pdo.jpeg", width= 30, height= 20, units= "cm", path = path_plot)
```
