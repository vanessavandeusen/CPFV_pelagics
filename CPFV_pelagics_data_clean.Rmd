...
Data input: cpfv_merged_uniqueID.csv; blocks.csv; MFDE_port_historic.csv; MFDE_Species_historic.csv
----------
...
species_DF<-read_csv("//10.74.13.28/ci-users/Vanessa.Van.Deusen/R_projects/CPFV_pelagics/Data/Cleaning_Keys/CDFW_species_key_Free_2.26.csv")
###Column names: "Species Name"     "Species ID"       "Species Group"    "CCL Category"     "Discontinue Date" "...6"



#____________________
#Importing data_clean.RDS to run final subsetting code section-- NOT possible/necessary when running initally
data_clean <- readRDS("//10.74.13.28/ci-users/Vanessa.Van.Deusen/R_projects/CPFV_pelagics/Data/CDFW_1980_2020_cpfv_logbook_data.Rds")
```
...
port_key<- port_DF %>% 
  ...
  dplyr::dplyr::select(-c(x5))


species_key<-species_DF %>% 
  clean_names("snake") %>%
  rename("species_code"= "spp_code_chr", 
         "species_discontinue_date"= "discontinue_date", 
         "species_name"= "sci_name")%>% 
  dplyr::dplyr::select(-c("spp_code_num"))%>%
...
####Number of Nas
```{r}
na_summary_cpfv_raw <- cpfv_merged_uniqueID %>%
  summarise(
    na_PortCode = sum(is.na(PortCode)), 
    na_HoursFished= sum(is.na(HoursFished)),
    na_Depth= sum(is.na(Depth)), 
    na_NumberReleased= sum(is.na(NumberReleased)), 
    na_NumberOfCrewFished= sum(is.na(NumberOfCrewFished)), 
    na_Block= sum(is.na(Block)),
    na_Species = sum(is.na(Species )),
    na_NumberOfFishers= sum(is.na(NumberOfFishers)),
    na_NumberKept= sum(is.na(NumberKept)),
    na_id_vessel = sum(is.na(id_vessel)),
    na_HoursMinutesFished = sum(is.na(HoursMinutesFished)),
    na_year = sum(is.na(day)),
    na_SpeciesCode= sum(is.na(SpeciesCode))
  )
View(na_summary_cpfv_raw)
```

```{r}
sum(is.na(cpfv_merged_uniqueID$HoursFished))
#13,707--0.3% of trips

sum(is.na(cpfv_merged_uniqueID$HoursFished) & is.na(cpfv_merged_uniqueID$HoursMinutesFished))
#10,949

sum(is.na(cpfv_merged_uniqueID$HoursMinutesFished))
#10949-- Seems like should use this one 

hist(cpfv_merged_uniqueID$HoursFished)
```
##Looking to see what may pose problems when ensuring numeric values are in numeric format
```{r}
cols_to_check <- c("LogMonth", "LogYear", "PortCode", "Block", "Depth", "Temperature", 
                   "NumberOfFishers", "NumberOfFishCaughtByCrew", "NumberOfCrewFished", "HoursFished", 
                   "NumberKept", "NumberReleased", "SpeciesCode")

# 2. Re-run the loop with the fix
for (col in cols_to_check) {
  
  # Use .data[[col]] to force R to look at the column, not the function
  offenders <- cpfv_merged_uniqueID %>%
    filter(!is.na(.data[[col]])) %>%
    filter(is.na(as.numeric(as.character(.data[[col]])))) %>%
    distinct(.data[[col]]) %>%
    pull(.data[[col]])
  
  if (length(offenders) > 0) {
    cat("\n--- Problem values in column:", col, "---\n")
    print(offenders)
  } else {
    cat("\nColumn", col, "is clean.\n")
  }
}
#issue is oarfish (1/8)
```
...
```{r}
target_n <- rev(sort(nrow(cpfv_merged_uniqueID)-freeR::complete(cpfv_merged_uniqueID %>% dplyr::select(TargetSpeciesLingcod:TargetSpeciesMiscBay))))
head(target_n)
```

## Which fishing method is most frequently used by recreational fishers?
```{r}
method_n <- rev(sort(nrow(cpfv_merged_uniqueID)-freeR::complete(cpfv_merged_uniqueID %>% dplyr::select(FishingMethodTrolling:FishingMethodOther))))
head(method_n)
```
## Which bait is most frequently used by recreational fishers?
```{r}
bait_n <- rev(sort(nrow(cpfv_merged_uniqueID)-freeR::complete(cpfv_merged_uniqueID %>% dplyr::select(BaitUsedAnchoviesLive:BaitUsedOtherDead))))
head(bait_n)
```
...
#Step 1.1: Initial Column Standardization and Name Cleaning --------
  ...
  mutate(
    #Convert characters to Date and time objects
    ...
    across(.cols= c(month, year, port_code, block_id, depth_ft, temp_f, n_fishers, n_caught_by_crew, 
               n_crew_fished, hrs_fished, n_kept, n_released, species_code),
           .fns= as.numeric), 
...
# Step 2.1: Spatial Joins-- Ports and Blocks
  ...
  left_join(block_key %>% 
              dplyr::select("block_id", "block_areaMi", "block_Acres"), 
            by="block_id") %>%
  
  # Step 5: Species Key Joins -- inner key removes entry that doesn't have a valid species code
  inner_join(species_key %>% dplyr::select(species_code, species_name, species_discontinue_date), 
            by=c("species_code")) %>%
...
####    mutate(species_name=ifelse(species_code==2665 , "Oarfish", species_name))%>%
   mutate(
    ...
    mutate(across(c(target_species, fishing_method, bait_used), #Removes the NAs within the string 
                  ~gsub("NA, |, NA", "", .x))) %>%
#___Removing Nas and Non-match Codes_______________________________
# Step 4.1 Removing Entries that Don't Match at Species Key
  filter(!is.na(hm_fished)) %>% #remove entries that don't have a time component
      
#___Consolidating Columns______________________   
# Step 5.1: Calculating total fishers, total catch, and days fished 
  mutate(
...
# Step 5.2: ID generation and column re-ordering  
  mutate(ID= row_number(), 
         logbook_id_use = paste(ID, date, id_vessel, sep="-")
         )%>%
  dplyr::select(logbook_id_use, 
...
```
Na introduced by coercion-- because oarfish 1/8 is entered in one row
#********************
##### :( STUCK HERE-- HM_FISHED IS STILL CODING AS A NUMBER RATHER THAN STAYING A TIME, NEED TO CHANGE HOW CALCULATE DAYS FISHED 
#******************
####QAQC
```{r}
missing_species <- data_clean %>%
  anti_join(species_key, by = "species_code") %>%
  distinct(species_code) #
View(missing_species)


na_summary_data_clean <- data_clean %>%
  summarise(
    na_species_name = sum(is.na(species_name)), 
    na_species_code= sum(is.na(species_code)),
    na_id_vessel= sum(is.na(id_vessel)), 
    na_n_released= sum(is.na(n_released)), 
    na_n_kept= sum(is.na(n_kept)), 
    na_hrs_fished= sum(is.na(hrs_fished)),
    na_hm_fished = sum(is.na(hm_fished )),
    na_total_fisher= sum(is.na(total_fishers)),
    na_total_caught= sum(is.na(total_caught)),
    na_year = sum(is.na(year)),
    na_year = sum(is.na(month)),
    na_year = sum(is.na(day)),
    na_hrs_and_hm= sum(is.na(hrs_fished) & is.na(hm_fished)), 
    na_species_name_and_code= sum(is.na(species_code) & is.na(species_name))
  )
View(na_summary_data_clean)
```
...
port_key_check <- data_clean %>% 
  dplyr::select(port_complex, port_code, port) %>% 
  unique() %>% 
  arrange(port_code)
...
species_key_check <- data_clean %>% 
  dplyr::select(species_code,species_name, comm_name_orig) %>% 
  unique()
...
duration_key <- data_clean %>% 
  dplyr::select(hm_fished, hrs_fished) %>% 
  unique() %>% 
  arrange(hrs_fished)
...
835 on-shore and off-shore blocks are located between California to Washington, 546 of those blocks are nearshore California. 29 international blocks extend down the coast of the Americas
##### Creating block trim list 
```{r}
CA_nearshore_blocks <- pull(blocks, var= BLOCK10_ID)
```

#####Trimming
```{r}
data_clean_CA_nearshore <- data_clean %>%
  filter(block_id %in% CA_nearshore_blocks)
```

#####QAQC
```{r}
length(unique(data_clean_CA_nearshore$block_id))
#545
```
...
###dplyr::selecting pelagics species 
```{r}
...
CPFV_pelagic_subset<- data_clean_CA_nearshore %>%
  ...
  dplyr::dplyr::select(
    "logbook_id_use", "year", "month", "day",
    "total_caught", "total_fishers", "days_fished", "port", "target_species", "fishing_method", 
    "bait_used", "block_id", "depth_ft", "species_name", "n_kept", "n_released", "id_vessel", 
    "n_caught_by_crew", "species_type", "CPUE_p"
         )
```  

###QAQC 
```{r}
na_summary_data_clean_pelagic_CA <- CPFV_pelagic_subset %>%
  summarise(
    na_cpue = sum(is.na(CPUE_p)), 
    na_species_type = sum(is.na(species_type)), 
    na_id_vessel= sum(is.na(id_vessel)), 
    na_n_released= sum(is.na(n_released)), 
    na_n_kept= sum(is.na(n_kept)), 
    na_days_fished= sum(is.na(days_fished)), 
    na_total_fisher= sum(is.na(total_fishers)),
    na_total_caught= sum(is.na(total_caught)),
    na_year = sum(is.na(year)),
    na_year = sum(is.na(month)),
...
View(na_summary_data_clean_pelagic_CA)
```
