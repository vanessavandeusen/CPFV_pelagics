...
PI: Jenny Selgrath
data input: cpfv_merged_no_pii.csv; blocks.csv; port_key.csv; species_key.csv
...
port_key <- read_csv("//10.74.13.28/ci-users/Vanessa.Van.Deusen/R_projects/CPFV_pelagics/Data/port_key.csv")
species_key<-read_csv("//10.74.13.28/ci-users/Vanessa.Van.Deusen/R_projects/CPFV_pelagics/Data/sp_list_key.csv")
```

## Formatting data
```{r}
block_key <- blocks %>% sf::st_drop_geometry()%>% rename("block_id"= "BLOCK10_ID", "block_areaMi"= "AreaMi", "block_Acres"="Acres", "block_shape_area"= "Shape__Area", "block_shape_length"= "Shape__Length")
port_key<- port_key %>% rename(port_code= PortID, port= PortName, port_complex= PortGroup_IOPAC)
port_key<-na.omit(port_key)
species_key<-species_key %>% rename("species_code"= "SpeciesID")%>% clean_names("snake")
...
```{r}
 data_clean <- cpfv_merged_no_pii %>% 
  ...
  mutate(species_code=recode(species_code, 
                             "Oarfish"="") %>% as.numeric(.)) %>%
  left_join(species_key %>% select(species_code, species_name), by=c("species_code")) %>%
