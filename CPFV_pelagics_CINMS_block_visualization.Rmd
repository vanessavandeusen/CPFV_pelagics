...
plot1<- ggplot()+ 
  geom_line(data=pelagicsCINMS_year_rm, aes(x=year, y=CPUE),size=1, color= "black")+
  theme_classic()+
  theme(axis.text.x = element_text(angle = 90, vjust = 1, hjust=1))+
  theme(legend.position="top")+
  theme(legend.title=element_blank())+
...
plotstacked
ggsave("CINMS_stacked_gas_all.jpeg", plot= plotstacked, width = 7, height = 8, units="in", path = path_plot)
```
...
plot3<-ggplot()+ 
  ...
  geom_sf(data=CINMS_shpfile_blocks, fill= "darkblue", alpha=0.25, color="black")+
...
###Path
```{r}
path_plot_heatmap<-"\\\\10.74.13.28/ci-users/Vanessa.Van.Deusen/R_projects/CPFV_pelagics/Data/graphs/CINMS/CINMS_heatmaps_by_year/independent_scale"
...
for(i in unique(CINMS_fishing_blocks_pelagic_2004$year)){
  x<-ggplot()+
     ...
     scale_fill_gradientn(colours=rev(magma(6)),
                         ...
                         na.value = "grey100", limits= c(0, 500))+
     theme_classic()+
  ...
  ggsave(x, path= path_plot_heatmap, file=paste0("CINMS_heatmap_individual_", i, ".png") )
}
```
