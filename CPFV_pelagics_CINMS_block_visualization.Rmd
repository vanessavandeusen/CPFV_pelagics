...
##Aggregating by year 
```{r}
pelagicsCINMS_year_rm<-aggregate(CPFV_pelagics_allblocks_rm$n_kept, by=list(year=CPFV_pelagics_allblocks_rm$year), FUN=sum)
pelagicsCINMS_year_rm<-rename(pelagicsCINMS_year_rm, "total"= "x")
```

##Aggregating by year and block
```{r}
##aggregating by block and year
pelagicsCINMS_year_block_rm<-aggregate(CPFV_pelagics_allblocks_rm$n_kept, by=list(year=CPFV_pelagics_allblocks_rm$year, block_id=CPFV_pelagics_allblocks_rm$block_id), FUN=sum)
pelagicsCINMS_year_block_rm<-rename(pelagicsCINMS_year_block_rm, "total"= "x")
pelagicsCINMS_year_block_rm<- replace(pelagicsCINMS_year_block_rm, is.na(pelagicsCINMS_year_block_rm), 0)
```

##Aggregating by species, year, block
```{r}
pelagicsCINMS_species_year_rm<-aggregate(CPFV_pelagics_allblocks_rm$n_kept, by=list(year=CPFV_pelagics_allblocks_rm$year, species=CPFV_pelagics_allblocks_rm$species_name, species_type=CPFV_pelagics_allblocks_rm$species_type, block_id= CPFV_pelagics_allblocks_rm$block_id), FUN=sum)
pelagicsCINMS_species_year_rm<-rename(pelagicsCINMS_species_year_rm, "total"= "x")
pelagicsCINMS_species_year_rm$year<-as.character(pelagicsCINMS_species_year_rm$year)

```

##Aggregating by species type, year
```{r}
pelagicsCINMS_species_year_type_rm<-aggregate(pelagicsCINMS_species_year_rm$total, by=list(species_type=pelagicsCINMS_species_year_rm$species_type, year=pelagicsCINMS_species_year_rm$year), FUN=sum)
pelagicsCINMS_species_year_type_rm<-rename(pelagicsCINMS_species_year_type_rm, "total"= "x")
```
...
ggplot(pelagicsCINMS_species_year_type_rm, aes(year, total), 
       )+ 
  ...
  #adding lines for mpa designation years
  geom_vline(aes(xintercept = 2004), pelagicsCINMS_species_year_type_rm, colour="black", size=1)+
  geom_vline(aes(xintercept = 2007), pelagicsCINMS_species_year_type_rm, colour="black", size=1)+
  #adding MHW events 
  geom_vline(aes(xintercept = 2015), pelagicsCINMS_species_year_type_rm, colour="black", size=0.75, linetype=2)+
  geom_vline(aes(xintercept = 2006), pelagicsCINMS_species_year_type_rm, colour="black", size=0.75, linetype=2)+
  geom_vline(aes(xintercept = 1995), pelagicsCINMS_species_year_type_rm, colour="black", size=0.75, linetype=2)+
  #adding lines where ENSO over 1.5 
  annotate("rect", xmin = 1982, xmax = 1983, ymin = -Inf, ymax = Inf, fill= "blue",
        alpha = .2)+
  annotate("rect", xmin = 1986, xmax = 1987, ymin = -Inf, ymax = Inf, fill= "blue",
  ...
  annotate("rect", xmin = 2015, xmax = 2016, ymin = -Inf, ymax = Inf, fill= "blue",
        alpha = .2)+
  #adding shading where PDO is over 1 
  geom_vline(aes(xintercept = 1983), pelagicsCINMS_species_year_type_rm, colour="black", size=0.75, linetype=3)+
  geom_vline(aes(xintercept = 1986), pelagicsCINMS_species_year_type_rm, colour="black", size=0.75, linetype=3)+
  geom_vline(aes(xintercept = 1993), pelagicsCINMS_species_year_type_rm, colour="black", size=0.75, linetype=3)+
  geom_vline(aes(xintercept = 1997), pelagicsCINMS_species_year_type_rm, colour="black", size=0.75, linetype=3)+
labs(title = str_wrap("CPFV catch in CINMS fishing blocks", 70))
ggsave("CINMS_line_pelagic.jpeg", width= 30, height= 20, units= "cm", path = path_plot)
```
