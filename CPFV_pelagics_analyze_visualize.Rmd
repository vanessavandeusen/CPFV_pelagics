...
```
###Troubleshooting double graph
```{r}

ggplot()+
  geom_col(data=pelagicsCINMS_species_block_year, aes(year, total, fill=species_type))+
  geom_line(data=gas_adjusted, aes(x=Year, y=adjust_price*6000))+
  scale_y_continuous("total", sec.axis = sec_axis( ~.*0.00006))

ggplot(gas_adjusted, aes(Year, adjust_price))+
  geom_line()
ggplot(gas_adjusted, aes(Year, adjust_price))+
  geom_line()
```
